...
const MyRoommateCard = ({ item: listing }) => {
  ...
  const imageUrl =
    listing.images[0] && `http://127.0.0.1:8000/storage/${listing.images[0]}`;
  ...
  const handlePressRight = () => {
    if (activeIndex === listing.images.length - 1)
      return flatListRef.current?.scrollToIndex({
        animated: false,
        index: 0,
      });
  ...
  const handleEditListing = () => {
    navigation.navigate("Edit Roommate Screen", {
      id: listing.id,
      token: user.token,
    });
  };
    ...
    <View className="flex flex-row mb-8 group">
      <View className="flex flex-col w-full gap-2">
        ...
        <View className="relative w-full overflow-hidden aspect-square rounded-xl">
          ...
          <View className="absolute top-3 right-3">
            <View className="relative transition hover:opacity-80">
              {listing.live_at === "" || listing.live_at === null ? (
                ...
                <View className="flex-row items-center px-3 py-1 rounded-full gap-x-2 text-emerald-500 bg-emerald-100/60">
                  ...
                  <Text className="text-sm font-normal">
                    Live at {moment(listing.live_at).format("MMM DD, YYYY")}
                  </Text>
                </View>
              )}
            </View>
        ...
        <View className="flex flex-row items-start justify-between mt-4">
          <View>
            <Text className="text-sm text-gray-800">{listing.title}</Text>

            <Text className="text-sm text-gray-800">
              Created at{" "}
              <Text className="font-semibold">
                {moment(listing.created_at).format("MMM DD, YYYY")}
              </Text>
            </Text>
          </View>
        </View>
